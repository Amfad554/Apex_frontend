...
const PROFESSIONAL = {
  tier:        'Professional',
  planKey:     'professional',        // ← sent to backend in metadata
  price:       '19,900',
  rawPrice:    19900,
  currency:    '₦',
...
function PaymentModal({ plan, onClose }) {
  ...
  // Step 2 — send Paystack's confirmed reference to our backend
  const verifyPayment = async (reference) => {
    ...
    try {
      ...
      if (!res.ok) {
        setStatus('error');
        setErrMsg(data.error || 'Verification failed. Please contact support.');
        return;
      }
      setStatus('success');
    } catch {
      setStatus('error');
      setErrMsg(`Network error during activation. Contact support with your reference: ${reference}`);
    }
  };
  ...
  const handlePay = () => {
    ...
    if (!paystackReady || !window.PaystackPop) { setErrMsg('Payment system is loading. Try again in a moment.'); return; }
    if (!token)                              { setErrMsg('You must be logged in to subscribe.'); return; }
    ...
    const handler = window.PaystackPop.setup({
      ...
      metadata: {
        planKey:   plan.planKey,          // ← backend reads this to identify the plan
        hospital:  storedUser?.hospitalName || '',
        adminName: storedUser?.adminName    || '',
      },
      callback: (response) => {
        setPaying(false);
        // Use the reference Paystack returns — not the one we generated
  ...
  const card = {
    ...
    width: '100%', maxWidth: 440, padding: '40px 32px', position: 'relative',
  };
  const inputBase = {
      ...
      <div style={card}>
        ...
        {status === 'success' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: '1.5rem', fontWeight: 900, color: C.navy, margin: '0 0 10px' }}>Payment Successful!</h2>
        ...
        {status === 'error' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: '1.4rem', fontWeight: 900, color: C.navy, margin: '0 0 10px' }}>Activation Failed</h2>
            <p style={{ color: '#6b7a99', fontSize: '0.9rem', lineHeight: 1.6, marginBottom: 24 }}>{errMsg}</p>
            <button onClick={() => { setStatus('idle'); setErrMsg(''); }} style={{ width: '100%', padding: '14px', background: C.orange, color: '#fff', border: 'none', borderRadius: 12, fontWeight: 700, cursor: 'pointer', fontSize: '0.95rem' }}>
              Try Again
            </button>
          </div>
        )}
        ...
        {status === 'verifying' && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <h2 style={{ fontSize: '1.3rem', fontWeight: 800, color: C.navy, marginBottom: 8 }}>Activating Your Plan…</h2>
            <p style={{ color: '#6b7a99', fontSize: '0.9rem' }}>Confirming your payment. Please don't close this window.</p>
          </div>
        )}
          ...
          <>
            ...
            <div style={{ background: C.lightGray, borderRadius: 16, padding: '18px 20px', marginBottom: 22 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10, borderTop: '1px solid rgba(0,0,0,0.06)' }}>
                ...
                <span style={{ fontSize: '1.2rem', fontWeight: 900, color: C.orange }}>
                  {plan.currency}{plan.price}<span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#94a3b8' }}>/mo</span>
                </span>
              </div>
            </div>
            ...
            <button
              ...
              style={{
                ...
                color: '#fff', border: 'none', borderRadius: 16, fontWeight: 800, fontSize: '1rem',
                cursor: paying ? 'not-allowed' : 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
                boxShadow: `0 8px 24px ${C.orange}40`, transition: 'transform .15s, box-shadow .15s',
              }}
