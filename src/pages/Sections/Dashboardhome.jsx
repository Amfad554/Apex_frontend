...
function useCountUp(target, duration = 900, enabled = true) {
    ...
    useEffect(() => {
        ...
        const step = (ts) => {
            ...
            // Ease-out cubic so the number slows down as it approaches the target
...
function Sparkline({ data, color, width = 80, height = 30, animate = false }) {
    ...
    // Normalise data points to fit within the SVG viewport
    const max = Math.max(...data), min = Math.min(...data), range = max - min || 1;
    const pts = data.map((v, i) =>
        `${(i / (data.length - 1)) * width},${height - ((v - min) / range) * height}`
    ).join(' ');

    // Animate the line drawing in using stroke-dashoffset
    useEffect(() => {
...
/* ─── LineGraph — multi-dataset SVG line chart for the "Patient Trends" card ─── */
function LineGraph({ datasets, labels, height = 160, animate = false, t }) {
    const W   = 400;
    ...
    // Convert each data value into SVG [x, y] coordinates
    const pts = (data) => data.map((v, i) => [
        pad.l + (i / (data.length - 1)) * gw,
        pad.t + gh - (v / max) * gh,
    ]);

    const lineRefs = useRef([]);

    // Animate each line drawing in with a staggered delay
    useEffect(() => {
        if (!animate) return;
...
function DonutChart({ value, total, color, size = 86, animate = false, t }) {
    ...
    return (
        <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
            ...
            {/* Foreground arc — animates from 0 to the calculated dash length */}
...
function StatCard({ label, value, gradient, icon: Icon, spark, delay, isMobile }) {
    ...
    // Stagger the card entrance using the `delay` prop
    useEffect(() => {
    ...
    return (
        ...
        >
            ...
            {/* Label, animated value, and sparkline */}
...
export default function DashboardHome({ isDark, t, hospital, onNavigate, isMobile }) {
    ...
    const [chartsReady,  setCharts]   = useState(false);  // controls chart animation trigger
    const [bottomReady,  setBottom]   = useState(false);  // controls bottom row animation trigger
    ...
    /* ── Stagger chart and bottom-row animations after data loads ── */
    useEffect(() => {
        if (loading) return;
    ...
    /* ── Pull values out of stats (default to 0 if not yet loaded) ── */
    ...
    return (
        <div style={{ color: t.text, fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
            ...
            {/* ── Stat cards row — each animates in with a staggered delay ── */}
            ...
            {/* ── Charts row — fades in after a short delay ── */}
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '3fr 2fr', gap: isMobile ? 10 : 20, marginBottom: isMobile ? 20 : 28, opacity: chartsReady ? 1 : 0, transform: chartsReady ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 0.5s ease,transform 0.5s ease' }}>
                ...
                <div style={{ background: t.card, borderRadius: 18, padding: isMobile ? 16 : 24, border: `1px solid ${t.border}`, boxShadow: '0 2px 12px rgba(10,26,63,0.06)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                        ...
                        {/* Legend rows — each slides in with a stagger */}
                        <div style={{ flex: 1 }}>
                            {[
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.6fr 1fr', gap: isMobile ? 10 : 20, opacity: bottomReady ? 1 : 0, transform: bottomReady ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 0.5s ease,transform 0.5s ease' }}>
                ...
                <div style={{ background: t.card, borderRadius: 18, padding: isMobile ? 16 : 24, border: `1px solid ${t.border}`, boxShadow: '0 2px 12px rgba(10,26,63,0.06)' }}>
                    ...
                    {/* Today's scheduled appointments — shows up to 3 */}
