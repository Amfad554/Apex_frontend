...
function Sparkline({ data, color, width = 80, height = 30, animate = false }) {
    ...
    return (
        <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
            <polyline ref={pathRef} points={pts} fill="none" stroke={color}
        ...
        <svg viewBox={`0 0 ${W} ${height}`} style={{ width: '100%', height,
            ...
            {[0, .25, .5, .75, 1].map((f, i) => (
                <line key={i} x1={pad.l} x2={W - pad.r}
                    y1={pad.t + gh * (1 - f)} y2={pad.t + gh * (1 - f)}
            ...
            {labels.map((l, i) => (
                <text key={i} x={pad.l + (i / (labels.length - 1)) * gw} y={height - 4}
                    textAnchor="middle" fill="rgba(255,255,255,0.30)" fontSize="10">{l}</text>
            ))}
        </svg>
    );
}
...
export default function DashboardHome({ hospital, onNavigate }) {
    ...
    if (loading) return (
        <div style={{ ...pageStyle, display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300, gap: 12, color: PALETTE.navySub }}>
        ...
        <div style={pageStyle}>
            ...
            <div style={{
                ...
                <div style={{ background: t.card, borderRadius: 18, padding: 24,
                    ...
                    <LineGraph
                        datasets={[
                            { data: [120,180,140,200,160,220,totalPts],         color: ACCENT.orange },
            ...
            <div style={{
                ...
                <div style={{ background: t.card, borderRadius: 18,
                    ...
                    <div style={{ padding: '20px 24px', display: 'flex',
                        ...
                        <button onClick={() => onNavigate('patients')} style={{
                            fontSize: 12, color: PALETTE.orange,
                    ...
                    {patients.length === 0 ? (
                        <div style={{ padding: 30, textAlign: 'center', color: PALETTE.navyMuted, fontSize: 14 }}>
                            No patients yet
                        </div>
                    ) : (
                ...
                <div style={{ background: t.card, borderRadius: 18, padding: 24,
                    ...
                    <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid ${t.border}` }}>
                        <p style={{ fontSize: 11, fontWeight: 600, color: PALETTE.navyMuted, marginBottom: 12,
                            textTransform: 'uppercase', letterSpacing: '0.05em' }}>Today's Appointments</p>
                            ...
                            return (
                                <div key={a.id} style={{
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        <p style={{ fontSize: 11, color: PALETTE.navyMuted }}>
                                            {a.doctor?.fullName} · {a.doctor?.department}
                                        </p>
                                    </div>
                                </div>
                            );
