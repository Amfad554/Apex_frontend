...
/* ─── Brand Tokens ─────────────────────────────────────────────────────────────
   Single source of truth — matches theme.js, PatientDashboard, HospitalDashboard
───────────────────────────────────────────────────────────────────────────── */
const ORANGE  = '#FF5A1F';
const ORANGE2 = '#e64d15';

const T = {
    bg:        '#0A1A3F',       // page / deepest navy
    card:      '#1F2A44',       // soft navy — cards, panels
...
function LineGraph({ datasets, labels, height = 160, animate = false }) {
    ...
    return (
        <svg viewBox={`0 0 ${W} ${height}`} style={{ width: '100%', height, animation: animate ? 'fadeInUp 0.5s ease both' : 'none' }}>
            ...
            {[0, .25, .5, .75, 1].map((f, i) => (
                <line key={i} x1={pad.l} x2={W - pad.r} y1={pad.t + gh * (1 - f)} y2={pad.t + gh * (1 - f)} stroke={T.border} strokeWidth="1" />
            ))}
            {datasets.map((ds, di) => {
                const p = pts(ds.data);
                return (
            ...
            {labels.map((l, i) => (
                <text key={i} x={pad.l + (i / (labels.length - 1)) * gw} y={height - 4} textAnchor="middle" fill={T.textMuted} fontSize="10">{l}</text>
            ))}
        </svg>
    );
}
...
function StatCard({ label, value, gradient, icon: Icon, spark, delay, isMobile }) {
    ...
    return (
        ...
        >
            ...
            <div
                style={{ width: isMobile ? 40 : 44, height: isMobile ? 40 : 44, borderRadius: 12, background: 'rgba(255,255,255,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.3s ease', flexShrink: 0, marginBottom: isMobile ? 0 : 16 }}
...
export default function DashboardHome({ hospital, onNavigate, isMobile }) {
    ...
    if (error) return (
        <div style={{ background: 'rgba(255,90,31,0.12)', border: `1px solid rgba(255,90,31,0.3)`, borderRadius: 12, padding: 20, color: ORANGE }}>
    ...
    return (
        <div style={{ color: T.text, fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
            ...
            <div style={{
                ...
                gap: isMobile ? 10 : 20, marginBottom: isMobile ? 20 : 28,
                opacity: chartsReady ? 1 : 0, transform: chartsReady ? 'translateY(0)' : 'translateY(20px)',
                ...
                <div style={{ background: T.card, borderRadius: 18, padding: isMobile ? 16 : 24, border: `1px solid ${T.border}`, boxShadow: T.shadow }}>
                    <div style={{ marginBottom: 20 }}>
                        <h3 style={{ fontWeight: 700, fontSize: 16, marginBottom: 8, color: T.text }}>Patient Trends</h3>
            ...
            <div style={{
                ...
                gap: isMobile ? 10 : 20,
                opacity: bottomReady ? 1 : 0, transform: bottomReady ? 'translateY(0)' : 'translateY(20px)',
                transition: 'opacity 0.5s ease,transform 0.5s ease',
            }}>
                {/* Recent Patients */}
                <div style={{ background: T.card, borderRadius: 18, border: `1px solid ${T.border}`, boxShadow: T.shadow, overflow: 'hidden' }}>
                    <div style={{ padding: isMobile ? '14px 16px' : '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${T.border}` }}>
                        ...
                        <button
                            ...
                            style={{ fontSize: 12, color: ORANGE, background: 'rgba(255,90,31,0.12)', border: 'none', padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 4, transition: 'background 0.2s,transform 0.15s' }}
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            ...
                            <tbody>
                                {patients.slice(0, 5).map((p, i) => {
                                    ...
                                    return (
                                        <tr key={p.id}
                                            style={{ borderBottom: `1px solid ${T.border}`, transition: 'background 0.15s,transform 0.15s', animation: bottomReady ? `rowFadeIn 0.35s ease both ${i * 0.07}s` : 'none', opacity: bottomReady ? undefined : 0 }}
                                        ...
                                        >
                                            <td style={{ padding: isMobile ? '10px 12px' : '12px 20px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                    <div
                                                        style={{ width: 32, height: 32, borderRadius: 9, background: color + '28', color, fontWeight: 700, fontSize: 11, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.2s ease' }}
