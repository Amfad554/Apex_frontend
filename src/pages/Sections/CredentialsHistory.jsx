...
const ROLE_COLOR = {
    doctor:'#18A8B5', nurse:'#34d399', pharmacist:'#a78bfa',
    lab_staff:'#fbbf24', receptionist:'#f472b6', patient:PALETTE.orange,
};
...
export default function CredentialsHistory({ isMobile }) {
    ...
    const ghostBtn = (extra = {}) => ({ display:'flex', alignItems:'center', gap:6, padding:'9px 14px', background:t.input, border:`1px solid ${t.border}`, borderRadius:10, color:t.textSub, fontWeight:600, fontSize:13, cursor:'pointer', fontFamily:'inherit', transition:'all 0.15s', ...extra });
        ...
        <div style={{ color:PALETTE.white, fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:28, gap:12, flexWrap:'wrap' }}>
                <div>
                    ...
                    <h1 style={{ fontSize: isMobile?20:24, fontWeight:800, letterSpacing:'-0.5px', marginBottom:4, color:PALETTE.white }}>Credentials Log</h1>
                ...
                <div style={{ display:'flex', gap:8 }}>
                    ...
                    {records.length > 0 && (
                        <button onClick={handleClearAll} style={ghostBtn({ background:confirmClear?'rgba(239,68,68,0.15)':'rgba(239,68,68,0.08)', border:`1px solid ${confirmClear?'rgba(239,68,68,0.4)':'rgba(239,68,68,0.2)'}`, color:'#ef4444' })}>
            ...
            <div style={{ display:'flex', gap:10, marginBottom:18, flexDirection: isMobile?'column':'row' }}>
                <div style={{ display:'flex', alignItems:'center', gap:8, background:t.input, borderRadius:10, padding:'8px 14px', border:`1px solid ${t.border}`, flex:1 }}>
                    ...
                    <input placeholder="Search by name, email, ID…" value={search} onChange={e => setSearch(e.target.value)} style={{ background:'none', border:'none', outline:'none', color:PALETTE.white, fontSize:13, width:'100%', fontFamily:'inherit' }} />
                            ...
                            <div style={{ padding:'14px 16px', display:'flex', alignItems:'center', gap:12, cursor:'pointer' }} onClick={() => setExpandedId(isExpanded?null:c.id)}>
                                ...
                                <div style={{ flex:1, minWidth:0 }}>
                                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3, flexWrap:'wrap' }}>
                                        ...
                                        {c.role && c.type === 'staff' && <span style={{ background:roleColor+'18', color:roleColor, fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20, textTransform:'capitalize' }}>{c.role.replace('_',' ')}</span>}
                                    </div>
                                ...
                                <div style={{ display:'flex', gap:6, alignItems:'center', flexShrink:0 }}>
                                    <button onClick={e => { e.stopPropagation(); copyWhatsApp(c); }} title="Copy WhatsApp message" style={{ display:'flex', alignItems:'center', gap:5, padding:'7px 12px', borderRadius:9, border:`1px solid ${isCopied?'rgba(16,185,129,0.4)':'rgba(255,90,31,0.35)'}`, background:isCopied?'rgba(16,185,129,0.1)':'rgba(255,90,31,0.1)', color:isCopied?'#10b981':PALETTE.orange, fontWeight:700, fontSize:12, cursor:'pointer', fontFamily:'inherit', transition:'all 0.2s', whiteSpace:'nowrap' }}>
                                        {isCopied ? <><CopyCheck size={13} /> Copied!</> : <><Share2 size={13} /> WhatsApp</>}
                                    </button>
                                    <button onClick={e => { e.stopPropagation(); remove(c.id); }} style={{ width:30, height:30, borderRadius:8, background:'rgba(239,68,68,0.08)', border:'none', cursor:'pointer', color:'#ef4444', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><Trash2 size={13} /></button>
