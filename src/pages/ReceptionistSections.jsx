...
function Badge({ status }) {
    ...
    return <span style={{ background:s.bg, color:s.color, fontSize:11, fontWeight:700,
        padding:'3px 10px', borderRadius:20, whiteSpace:'nowrap' }}>{s.label}</span>;
}
...
function Toast({ message, type='success', onClose }) {
    ...
    return (
        <div style={{ position:'fixed',top:20,right:20,zIndex:99999,background:c.bg,
...
function LoadingState({ t, accent }) {
    return (
        <div style={{ padding:60,display:'flex',flexDirection:'column',alignItems:'center',gap:14 }}>
            ...
            <div style={{ width:32,height:32,border:`2.5px solid ${accent}22`,
...
function StatCard({ label, value, icon:Icon, color, t }) {
    return (
        <div style={{ background:t.surface,borderRadius:16,padding:'18px',border:`1px solid ${t.border}`,
            ...
            <div style={{ width:38,height:38,borderRadius:11,background:color+'16',
                display:'flex',alignItems:'center',justifyContent:'center',marginBottom:10 }}>
                <Icon size={17} color={color}/>
            </div>
            <p style={{ fontSize:26,fontWeight:800,color:t.text,letterSpacing:'-0.5px',
                fontFamily:'monospace',marginBottom:2 }}>{value}</p>
...
function SearchBar({ value, onChange, placeholder, t }) {
    return (
        <div style={{ display:'flex',alignItems:'center',gap:8,background:t.surface,borderRadius:10,
            ...
            {value && <button onClick={()=>onChange('')} style={{ background:'none',border:'none',
                cursor:'pointer',color:t.textMuted,display:'flex' }}><X size={13}/></button>}
        </div>
    );
}
...
const rApi = {
    patients: {
        list:(hId,p={})=>{const q=new URLSearchParams(p).toString();
    ...
    billing: {
        ...
        create:(b)=>fetch(`${BASE_URL}/api/billing`,{method:'POST',headers:hdrs(),body:JSON.stringify(b)}).then(handle),
    ...
    admissions: {
        ...
        create:(b)=>fetch(`${BASE_URL}/api/admissions`,{method:'POST',headers:hdrs(),body:JSON.stringify(b)}).then(handle),
    ...
    queue: {
        ...
        add:(b)=>fetch(`${BASE_URL}/api/queue`,{method:'POST',headers:hdrs(),body:JSON.stringify(b)}).then(handle),
    ...
    lab: {
        ...
        create:(b)=>fetch(`${BASE_URL}/api/lab-requests`,{method:'POST',headers:hdrs(),body:JSON.stringify(b)}).then(handle),
...
export function BillingSection({ t, hospitalId, accent, isMobile, externalSearch='' }) {
    ...
    const inputStyle = { width:'100%',background:t.input,border:`1px solid ${t.border}`,borderRadius:10,
        ...
        <div>
                                            ...
                                            <tr key={b.id} style={{ borderBottom:`1px solid ${t.border}` }}
                                                ...
                                                <td style={{ padding:'12px 16px' }}>
                                                    <div style={{ display:'flex',alignItems:'center',gap:8 }}>
                                                        <div style={{ width:30,height:30,borderRadius:8,background:col+'22',
                                                    ...
                                                    <div style={{ display:'flex',gap:6 }}>
                                                        {b.status!=='paid' && (
                                                            <button onClick={()=>{ setPayForm({amount:'',method:'cash',notes:''}); setFormError(''); setShowPay(b); }}
                                                                style={{ padding:'4px 10px',background:`${EMERALD}15`,border:'none',
                                                        ...
                                                        <button onClick={()=>handleDelete(b.id)}
                                                            style={{ width:28,height:28,borderRadius:7,background:'rgba(239,68,68,0.08)',
            ...
            {showAdd && (
                <div onClick={e=>e.target===e.currentTarget&&setShowAdd(false)}
                    ...
                    <div style={{ background:t.surface,borderRadius:20,width:'100%',maxWidth:500,
                        ...
                        <div style={{ padding:'18px 20px',borderBottom:`1px solid ${t.border}`,display:'flex',
                            ...
                            <button onClick={()=>setShowAdd(false)} style={{ background:'rgba(239,68,68,0.1)',border:'none',
                        ...
                        <form onSubmit={handleAdd} style={{ padding:20 }}>
                            {formError && <div style={{ background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.2)',
                                borderRadius:10,padding:'10px 14px',color:'#ef4444',fontSize:13,marginBottom:16 }}>{formError}</div>}
                            ...
                            <div style={{ display:'flex',gap:10,marginTop:20 }}>
                                <button type="button" onClick={()=>setShowAdd(false)} style={{ flex:1,padding:'11px',
                                    background:t.input,border:`1px solid ${t.border}`,borderRadius:10,
                                    color:t.textSub,fontWeight:600,cursor:'pointer',fontFamily:'inherit',fontSize:14 }}>Cancel</button>
                                <button type="submit" disabled={submitting} style={{ flex:2,padding:'11px',background:ORANGE,
                                    border:'none',borderRadius:10,color:'#fff',fontWeight:700,
                                    cursor:submitting?'not-allowed':'pointer',fontFamily:'inherit',fontSize:14,
                                    opacity:submitting?0.7:1 }}>{submitting?'Creating...':'Create Invoice'}</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {showPay && (
                <div onClick={e=>e.target===e.currentTarget&&setShowPay(null)}
                    ...
                    <div style={{ background:t.surface,borderRadius:20,width:'100%',maxWidth:440,
                        ...
                        <div style={{ padding:'18px 20px',borderBottom:`1px solid ${t.border}`,display:'flex',
                            ...
                            <button onClick={()=>setShowPay(null)} style={{ background:'rgba(239,68,68,0.1)',border:'none',
                        ...
                        <form onSubmit={handlePayment} style={{ padding:20 }}>
                            ...
                            <div style={{ display:'flex',gap:10,marginTop:20 }}>
                                <button type="button" onClick={()=>setShowPay(null)} style={{ flex:1,padding:'11px',
                                    background:t.input,border:`1px solid ${t.border}`,borderRadius:10,
                                    color:t.textSub,fontWeight:600,cursor:'pointer',fontFamily:'inherit',fontSize:14 }}>Cancel</button>
                                <button type="submit" disabled={submitting} style={{ flex:2,padding:'11px',background:EMERALD,
                                    border:'none',borderRadius:10,color:'#fff',fontWeight:700,
                                    cursor:submitting?'not-allowed':'pointer',fontFamily:'inherit',fontSize:14,
                                    opacity:submitting?0.7:1 }}>{submitting?'Saving...':'Record Payment'}</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div>
            ...
            {loading ? <LoadingState t={t} accent={accent}/> : (
                <div style={{ display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(auto-fill,minmax(320px,1fr))',gap:14 }}>
                    {filtered.length===0
                        ? <div style={{ gridColumn:'1/-1',padding:40,textAlign:'center',color:t.textMuted,
                        ...
                        : filtered.map((a,i)=>{
                            const col = AVATAR_COLORS[i%AVATAR_COLORS.length];
                            return (
                                <div key={a.id} style={{ background:t.surface,borderRadius:16,padding:18,
                                    border:`1px solid ${t.border}`,boxShadow:t.shadow }}>
                                    <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12 }}>
                                        <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                                            <div style={{ width:38,height:38,borderRadius:10,background:col+'22',color:col,
                                                fontWeight:700,fontSize:13,display:'flex',alignItems:'center',
                                                justifyContent:'center',flexShrink:0 }}>{initials(a.patient?.fullName)}</div>
                                            <div>
                                    ...
                                    {a.reason && <p style={{ fontSize:12,color:t.textSub,marginBottom:10,lineHeight:1.5,
                                    ...
                                    <div style={{ display:'flex',gap:6 }}>
                                        {a.status==='admitted' && (
                                            <button onClick={()=>{ setDischargeNotes(''); setShowDischarge(a); }}
                                                style={{ flex:1,padding:'7px',background:`${EMERALD}12`,border:'none',borderRadius:8,
                                                    color:EMERALD,fontWeight:600,fontSize:12,cursor:'pointer',fontFamily:'inherit' }}>
                                                Discharge
                                            </button>
                                        )}
                                        <button onClick={()=>handleDelete(a.id)}
                                            style={{ width:30,height:30,borderRadius:8,background:'rgba(239,68,68,0.08)',border:'none',
            ...
            {showAdd && (
                <div onClick={e=>e.target===e.currentTarget&&setShowAdd(false)}
                    style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.65)',zIndex:9999,
                        overflowY:'auto',padding:'40px 20px',backdropFilter:'blur(12px)',WebkitBackdropFilter:'blur(12px)' }}>
                    <div style={{ background:t.surface,borderRadius:20,width:'100%',maxWidth:520,
                        border:`1px solid ${t.border}`,boxShadow:'0 24px 80px rgba(0,0,0,0.5)',margin:'0 auto',marginBottom:40 }}>
                        <div style={{ padding:'18px 20px',borderBottom:`1px solid ${t.border}`,display:'flex',
                            justifyContent:'space-between',alignItems:'center',borderTop:`3px solid ${ORANGE}` }}>
                        ...
                        <form onSubmit={handleAdmit} style={{ padding:20 }}>
                            ...
                            <div style={{ display:'flex',gap:10,marginTop:20 }}>
                                ...
                                <button type="submit" disabled={submitting} style={{ flex:2,padding:'11px',background:ORANGE,
                                    border:'none',borderRadius:10,color:'#fff',fontWeight:700,
                                    cursor:submitting?'not-allowed':'pointer',fontFamily:'inherit',fontSize:14,
                                    opacity:submitting?0.7:1 }}>{submitting?'Admitting...':'Admit Patient'}</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {showDischarge && (
                <div onClick={e=>e.target===e.currentTarget&&setShowDischarge(null)}
                    style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.65)',zIndex:9999,
                        display:'flex',alignItems:'center',justifyContent:'center',padding:20,
                        backdropFilter:'blur(12px)',WebkitBackdropFilter:'blur(12px)' }}>
                    <div style={{ background:t.surface,borderRadius:20,width:'100%',maxWidth:420,
                        border:`1px solid ${t.border}`,boxShadow:'0 24px 80px rgba(0,0,0,0.5)' }}>
                        <div style={{ padding:'18px 20px',borderBottom:`1px solid ${t.border}`,display:'flex',
                            justifyContent:'space-between',alignItems:'center',borderTop:`3px solid ${EMERALD}` }}>
                            <div>
                            ...
                            <button onClick={()=>setShowDischarge(null)} style={{ background:'rgba(239,68,68,0.1)',border:'none',
                                borderRadius:8,cursor:'pointer',color:'#ef4444',width:30,height:30,
                                display:'flex',alignItems:'center',justifyContent:'center' }}><X size={15}/></button>
                        </div>
                        <form onSubmit={handleDischarge} style={{ padding:20 }}>
                            <div>
                                ...
                                <textarea style={{ ...inputStyle,minHeight:80,resize:'vertical',width:'100%',
                            ...
                            <div style={{ display:'flex',gap:10,marginTop:20 }}>
                                <button type="button" onClick={()=>setShowDischarge(null)} style={{ flex:1,padding:'11px',
                                    background:t.input,border:`1px solid ${t.border}`,borderRadius:10,
                                    color:t.textSub,fontWeight:600,cursor:'pointer',fontFamily:'inherit',fontSize:14 }}>Cancel</button>
                                <button type="submit" disabled={submitting} style={{ flex:2,padding:'11px',background:EMERALD,
                                    border:'none',borderRadius:10,color:'#fff',fontWeight:700,
                                    cursor:submitting?'not-allowed':'pointer',fontFamily:'inherit',fontSize:14,
                                    opacity:submitting?0.7:1 }}>{submitting?'Processing...':'Confirm Discharge'}</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div>
            ...
            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20,gap:12,flexWrap:'wrap' }}>
                ...
                <div style={{ display:'flex',gap:8 }}>
                    <button onClick={load} style={{ display:'flex',alignItems:'center',gap:6,padding:'10px 14px',
            ...
            {loading ? <LoadingState t={t} accent={accent}/> : (
                sortedQueue.length===0
                    ? <div style={{ padding:60,textAlign:'center',color:t.textMuted,background:t.surface,
                    ...
                    : <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
                        {sortedQueue.map((q,i)=>{
                            ...
                            return (
                                <div key={q.id} style={{ background:t.surface,borderRadius:14,padding:'14px 18px',
                                    ...
                                    {/* queue number */}
                                    <div style={{ width:42,height:42,borderRadius:12,background:`${ORANGE}18`,color:ORANGE,
                                    ...
                                    <div style={{ display:'flex',alignItems:'center',gap:10,flex:1,minWidth:0 }}>
                                        <div style={{ width:36,height:36,borderRadius:10,background:col+'22',color:col,
                                            fontWeight:700,fontSize:12,display:'flex',alignItems:'center',
                                            justifyContent:'center',flexShrink:0 }}>{initials(q.patient?.fullName)}</div>
                                        <div style={{ minWidth:0 }}>
                                            <p style={{ fontWeight:700,fontSize:14,overflow:'hidden',
                                                textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{q.patient?.fullName}</p>
                                    ...
                                    <div style={{ display:isMobile?'none':'block',minWidth:0,flex:1 }}>
                                        <p style={{ fontSize:12,color:t.textSub,overflow:'hidden',
                                            textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{q.reason}</p>
                                    </div>
                                    {/* badges */}
                                    <div style={{ display:'flex',gap:6,alignItems:'center',flexShrink:0 }}>
                                        <span style={{ background:pri.bg,color:pri.color,fontSize:11,fontWeight:700,
                                            padding:'3px 10px',borderRadius:20 }}>{pri.label}</span>
                                    ...
                                    <div style={{ display:'flex',gap:6,flexShrink:0 }}>
                                        {q.status==='waiting' && (
                                            <button onClick={()=>updateStatus(q.id,'called')}
                                                style={{ padding:'6px 12px',background:`${BLUE}15`,border:'none',borderRadius:8,
                                        ...
                                        {q.status==='called' && (
                                            <button onClick={()=>updateStatus(q.id,'in-progress')}
                                                style={{ padding:'6px 12px',background:`${VIOLET}15`,border:'none',borderRadius:8,
                                        ...
                                        {q.status==='in-progress' && (
                                            <button onClick={()=>updateStatus(q.id,'completed')}
                                                style={{ padding:'6px 12px',background:`${EMERALD}15`,border:'none',borderRadius:8,
            ...
            {showAdd && (
                <div onClick={e=>e.target===e.currentTarget&&setShowAdd(false)}
                    style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.65)',zIndex:9999,
                        display:'flex',alignItems:'center',justifyContent:'center',padding:20,
                        backdropFilter:'blur(12px)',WebkitBackdropFilter:'blur(12px)' }}>
                    <div style={{ background:t.surface,borderRadius:20,width:'100%',maxWidth:460,
                        border:`1px solid ${t.border}`,boxShadow:'0 24px 80px rgba(0,0,0,0.5)' }}>
                        <div style={{ padding:'18px 20px',borderBottom:`1px solid ${t.border}`,display:'flex',
                            justifyContent:'space-between',alignItems:'center',borderTop:`3px solid ${ORANGE}` }}>
                        ...
                        <form onSubmit={handleAdd} style={{ padding:20 }}>
                            ...
                            <div style={{ display:'flex',gap:10,marginTop:20 }}>
                                ...
                                <button type="submit" disabled={submitting} style={{ flex:2,padding:'11px',background:ORANGE,
                                    border:'none',borderRadius:10,color:'#fff',fontWeight:700,
                                    cursor:submitting?'not-allowed':'pointer',fontFamily:'inherit',fontSize:14,
                                    opacity:submitting?0.7:1 }}>{submitting?'Adding...':'Add to Queue'}</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div style={{ display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(auto-fill,minmax(300px,1fr))',gap:14 }}>
                    {filtered.length===0
                        ...
                        : filtered.map((r,i)=>{
                            ...
                            return (
                                <div key={r.id} style={{ background:t.surface,borderRadius:16,padding:16,
                                    border:`1px solid ${t.border}`,boxShadow:t.shadow }}>
                                    {/* top row */}
                                    <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10 }}>
                                        ...
                                        <div style={{ display:'flex',gap:6 }}>
                                            <span style={{ background:urg.bg,color:urg.color,fontSize:11,fontWeight:700,
                                                padding:'3px 10px',borderRadius:20 }}>{urg.label}</span>
                                            <button onClick={()=>setViewReq(r)}
                                                style={{ width:28,height:28,borderRadius:7,background:`${ORANGE}15`,border:'none',
                                    ...
                                    <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:10 }}>
                                        <div style={{ width:28,height:28,borderRadius:8,background:col+'22',color:col,
                                            fontWeight:700,fontSize:9,display:'flex',alignItems:'center',
                                            justifyContent:'center',flexShrink:0 }}>{initials(r.patient?.fullName)}</div>
                                        <div>
                                    ...
                                    <div style={{ display:'flex',gap:6 }}>
                                        {nextStatus && nextLabels[nextStatus] && (
                                            <button onClick={()=>updateStatus(r.id, nextStatus)}
                                                style={{ flex:1,padding:'7px',background:`${ORANGE}12`,border:'none',borderRadius:8,
                ...
                <div onClick={e=>e.target===e.currentTarget&&setViewReq(null)}
                    ...
                    <div style={{ background:t.surface,borderRadius:20,width:'100%',maxWidth:420,
                        ...
                        <div style={{ padding:'18px 20px',borderBottom:`1px solid ${t.border}`,display:'flex',
                            ...
                            <button onClick={()=>setViewReq(null)} style={{ background:'rgba(239,68,68,0.1)',border:'none',
                                borderRadius:8,cursor:'pointer',color:'#ef4444',width:30,height:30,
                                display:'flex',alignItems:'center',justifyContent:'center' }}><X size={15}/></button>
                        </div>
                        <div style={{ padding:20 }}>
                            <div style={{ marginBottom:14,display:'flex',gap:8 }}>
                                ...
                                {(() => { const u=URGENCY_COLORS[viewReq.urgency]||URGENCY_COLORS.routine;
                                    return <span style={{ background:u.bg,color:u.color,fontSize:11,fontWeight:700,
                                        padding:'3px 10px',borderRadius:20 }}>{u.label}</span>; })()}
                            </div>
                        ...
                        <form onSubmit={handleAdd} style={{ padding:20 }}>
                            ...
                            <div style={{ display:'flex',gap:10,marginTop:20 }}>
                                ...
                                <button type="submit" disabled={submitting} style={{ flex:2,padding:'11px',background:ORANGE,
                                    border:'none',borderRadius:10,color:'#fff',fontWeight:700,
                                    cursor:submitting?'not-allowed':'pointer',fontFamily:'inherit',fontSize:14,
                                    opacity:submitting?0.7:1 }}>{submitting?'Creating...':'Create Request'}</button>
                            </div>
                        </form>
                    </div>
                </div>
