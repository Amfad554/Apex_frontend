...
/* ─────────────────────────────────────────────────────────────────────────────
   API CONFIG — swap these URLs when your backend is ready
───────────────────────────────────────────────────────────────────────────── */
const API = {
    stats: `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/platform/stats`,
    contact: `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/contact`,
};
...
function useAnimatedStats(target, duration = 2000) {
    ...
    }, [target.patients]); // re-run when real data arrives
...
export default function Home() {
    ...
    const handleSubmit = async (e) => {
        ...
        const field = (key, label, type = "text", extra = {}) => (
            <FieldWrapper error={touched[key] && errors[key]}>
                {type === "select" ? (
                    <select
                        value={form[key]}
